...
class WorkingHours extends Model {
    ...
    public function getNextTime() {
        ...
        if(!$this->time4) return 'time5';
        if(!$this->time4) return 'time6';
        if(!$this->time4) return 'time7';
        if(!$this->time4) return 'time8';
        return null;
    }
    ...
    public function getActiveCLock() {
        ...
        } elseif ($nextTime === 'time5' || $nextTime === 'time7') {
                return 'exitTime';
            } elseif ($nextTime === 'time6' || $nextTime === 'time8') {
                return 'workedInterval';
            } else {
            return null;
        }
    ...
    public function innout($time) {
        ...
        if(!$timeColumn) {
            throw new AppException ("Você já fez os 8 batimentos do dia!");
        }
    ...
    function getWorkedInterval() {
        [$t1, $t2, $t3, $t4, $t5, $t6, $t7, $t8] = $this->getTimes();
        ...
        if($t5) $part3 = $t5->diff(new DateTime());
        if($t6) $part3 = $t5->diff($t6);
        if($t7) $part4 = $t7->diff(new DateTime());
        if($t8) $part4 = $t7->diff($t8);

        return sumIntervals($part1, $part2, $part3, $part4);
    }

    function getLunchInterval() {
        [, $t2, $t3, $t4, $t5, $t6, $t7, $t8] = $this->getTimes();
